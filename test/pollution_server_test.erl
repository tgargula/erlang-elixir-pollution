...
get_one_value_test() -> 
  ?assertEqual(ok, pollution_server:get_one_value("Station", {{2021,4,25}, {19,00,52}}, "PM10")),
  ?assertEqual(ok, pollution_server:get_one_value("Station 2", {{2021,4,25}, {17,27,52}}, "PM2.5")).

get_station_mean_test() -> 
  ?assertEqual(ok, pollution_server:get_station_mean("Station", "PM10")),
  ?assertEqual(ok, pollution_server:get_station_mean("Station 2", "PM2.5")).

get_daily_mean_test() -> 
  ?assertEqual(ok, pollution_server:get_daily_mean("PM10", {2021,4,25})),
  ?assertEqual(ok, pollution_server:get_daily_mean("PM2.5", {2021,4,25})).

get_maximum_variation_station_test() ->
  ?assertEqual(ok, pollution_server:get_maximum_variation_station("PM10")),
  ?assertEqual(ok, pollution_server:get_maximum_variation_station("PM2.5")).

get_station_variation_test() ->
  ?assertEqual(ok, pollution_server:get_station_variation("Station", "PM10")),
  ?assertEqual(ok, pollution_server:get_station_variation("Station 2", "PM2.5")).

get_stats_test() -> 
  ?assertEqual(ok, pollution_server:get_stats()).
